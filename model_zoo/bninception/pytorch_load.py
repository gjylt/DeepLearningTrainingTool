...
class BNInception(nn.Module):
    ...
    def __init__(self, model_path='model_zoo/bninception/bn_inception.yaml', num_classes=101,
        ...
        # inceptionWeight = torch.utils.model_zoo.load_url(weight_url)
        inceptionWeight = torch.load(weight_url)
        for layerName in inceptionWeight.keys():
            layerWeight = inceptionWeight[layerName]
            if len(layerWeight.shape) == 2 and layerWeight.shape[0] == 1:
                inceptionWeight[layerName] = inceptionWeight[layerName][0]
        self.load_state_dict(inceptionWeight)
